...
class AutodeskOAuth:
    ...
    def auth_status(self) -> dict[str, Any]:
        ...
        return ok({
            ...
            "expired": expired,
            "tokenUsable": bool(tokens.get("access_token") and not expired and not tokens.get("_store_corrupt")),
            "needsLogin": not configured or not bool(tokens.get("access_token")) or bool(tokens.get("_store_corrupt")),
            "pendingLoginPresent": self._pending_path.exists(),
            "tokenStore": str(self.store.path),
            "tokenStoreCorrupt": bool(tokens.get("_store_corrupt")),
            "lastRefreshError": redact(self._last_refresh_error) if self._last_refresh_error else None,
            "tokens": redact(tokens)
        })

    def start_login(self) -> dict[str, Any]:
        static_validation = self.config.validate_static_urls()
        ...
        self.store.write_private_json(self._pending_path, {
            "verifier": verifier,
            "state": state,
            "redirect_uri": self.config.redirect_uri,
            "created_at": time.time(),
            "ttl_sec": self.config.pending_login_ttl_sec
        })
        return ok({
            "authorizationUrl": url,
            "redirectUri": self.config.redirect_uri,
            "state": state,
            "expiresInSec": self.config.pending_login_ttl_sec,
    ...
    def complete_login(self, code: str | None = None, state: str | None = None) -> dict[str, Any]:
        ...
        if not state:
            return blocked("missing_oauth_state", "Pass the state returned by Autodesk and start_login. Restart login if the state was lost.")
        if not self._pending_path.exists():
            return blocked("missing_pending_login", "Call start_login before completing login.")
        pending = self.store.read_private_json(self._pending_path)
        if pending.get("_store_corrupt"):
            self._pending_path.unlink(missing_ok=True)
            return blocked("pending_login_corrupt", "Restart login; pending OAuth state could not be read.")
        if time.time() - float(pending.get("created_at", 0)) > self.config.pending_login_ttl_sec:
            self._pending_path.unlink(missing_ok=True)
            return blocked("pending_login_expired", "Restart login; pending OAuth state expired.")
        ...
        req = request.Request(self.config.token_url, data=data, method="POST", headers={"Content-Type": "application/x-www-form-urlencoded", "Accept": "application/json"})
        try:
            with request.urlopen(req, timeout=self.config.request_timeout_sec) as resp:
                tokens = json.loads(resp.read().decode())
        except error.HTTPError as exc:
            detail = exc.read().decode(errors="replace")[:1000]
            return blocked("token_exchange_failed", "Check Autodesk OAuth app redirect URI, scopes, and client id.", status=exc.code, detail=redact({"detail": detail})["detail"])
    ...
    def get_access_token(self) -> str | None:
        ...
        if token and not self._is_expired(tokens) and not tokens.get("_store_corrupt"):
            return token
        if tokens.get("refresh_token"):
            refreshed = self.refresh()
            if refreshed.get("ok"):
                return self.store.load().get("access_token")
            self._last_refresh_error = refreshed
            return None
        return None
    ...
    def refresh(self) -> dict[str, Any]:
        ...
        try:
            ...
            detail = exc.read().decode(errors="replace")[:1000]
            return blocked("refresh_failed", "Autodesk token refresh failed; restart login if this persists.", status=exc.code, detail=redact({"detail": detail})["detail"])
        except Exception as exc:  # noqa: BLE001
